...
float ClosestPoints(Point points[], int length, Point &a, Point &b)
{
...
int main()
{
	...
	else
	{
		...
		if (N == 1)
		{
			cout << "please input more than two points" << endl;
		}
		cout<<N<<"个二维点如下："<<endl; 
		for(i=0; i<N; i++) 
		{
		...
		cout << "test1" << endl;
		dist=ClosestPoints(points,N,a,b);
		cout << "test2" << endl;
		cout<<"最近点对为："<<"("<<a.x<<","<<a.y<<")和"<<"("<<b.x<<","<<b.y<<")"<<endl;
		cout<<"最近点对距离为："<<dist<<endl;
	}
}